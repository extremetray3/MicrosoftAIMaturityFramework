(function () {
  ...
  var STORAGE_PREFIX = 'aimat-notes-';
  ...
  function exportAllNotes() {
    ...
    lines.push('**Site:** Microsoft AI Maturity Framework');
    lines.push('');
    ...
    if (pageNames.length === 0) { alert('No notes to export.'); return; }
  ...
  function saveSession() {
    ...
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (k && (k.indexOf(STORAGE_PREFIX) === 0 || k.indexOf('aimat-maturity-') === 0)) {
    ...
    if (Object.keys(data).length === 0) { alert('No session data to save.'); return; }
    ...
    input.addEventListener('change', function () {
      ...
      reader.onload = function (e) {
        try {
          ...
          var validKeys = 0;
          for (var k in data) {
            if (k.indexOf(STORAGE_PREFIX) === 0 || k.indexOf('aimat-maturity-') === 0 || k === CUSTOMER_KEY) validKeys++;
          }
          if (validKeys === 0) { alert('This file does not contain valid session data.'); return; }
  ...
  function clearAllNotes() {
    try {
      ...
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (k && (k.indexOf(STORAGE_PREFIX) === 0 || k.indexOf('aimat-maturity-') === 0 || k === CUSTOMER_KEY)) {
          keysToRemove.push(k);
        }
      }
  ...
  function init() {
    ...
    for (var i = 0; i < headings.length; i++) {
      if (headings[i].closest('.notes-toolbar') || headings[i].closest('.notes-control') || headings[i].closest('.mc-container')) continue;
